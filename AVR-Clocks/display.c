...
static const uint8_t stddelay = 10;
static const uint16_t sym_delay = 1;
...
void segm_indicate(struct segm_Display *display, uint8_t *arr4, uint8_t pos)
{
	uint8_t sym;
	uint8_t digit;  /* [_ _ _ _ 4 3 2 1] -> 7SEGM:[4] [3] [2] [1] */
	for (int i = 0; i < 4; i++) {
		sym = arr4[i];
		if (!display->is_comm_anode)
			sym = ~sym;
		segm_shiftbyte(display, sym);
		/* (3-i) to display symbols from left to right */
		segm_shiftbyte(display, (pos & (1 << (3-i)))); 
		segm_latch(display);
